...
namespace red1z {
  namespace impl {
    ...
    public:
      ...
      //   for (int i = 0; i < n; i += n) {
      //     out.emplace(items[2*i].template as<K>().value(), items[2*i+1].template as<V>().value());
      //   }
      //   return out;
      // }

      template <class K, class F>
      decltype(auto) hincrby(K const& key, F const& field, std::int64_t increment) {
        return run(IntegerCommand("HINCRBY", key, field, std::to_string(increment)));
      }

      template <class K, class F>
      decltype(auto) hincrbyfloat(K const& key, F const& field, double increment) {
        return run(FloatCommand("HINCRBYFLOAT", key, field, std::to_string(increment)));
      }
      ...
      template <class K>
      decltype(auto) hlen(K const& key) { return run(IntegerCommand("HLEN", key)); }
      ...
      decltype(auto) hmget(K const& key, Fs const&... fields) {
        if constexpr(has_pack_v<Fs...>) {
          ...
          return run(TupleOptCommand<Ts...>("MGET", key, fields...));
        }
      }

      template <class K, class... FVs>
      decltype(auto) hmset(K const& key, FVs const&... field_value_pairs) {
        static_assert(pair_checker<FVs...>::value, "hmset(): invalid number of arguments");
        return run(StatusCommand("HMSET", key, field_value_pairs...));
      }

      template <class K, class... FVs>
      decltype(auto) hset(K const& key, FVs const&... field_value_pairs ) {
        static_assert(pair_checker<FVs...>::value, "hset(): invalid number of arguments");
        return run(IntegerCommand("HSET", key, field_value_pairs...));
      }
      ...
      decltype(auto) hvals(K const& key) {

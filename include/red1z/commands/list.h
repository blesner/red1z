...
namespace red1z {
  namespace impl {
    template <class Executor> class ListCommands {
    ...
    public:
      ...
      decltype(auto) blpop(Key const &key, Args const &... args) {
        ...
        return run(OptTupleCommand<K, V>("BLPOP", s(), key, args...));
      }
      ...
      decltype(auto) brpop(Key const &key, Args const &... args) {
        ...
        return run(OptTupleCommand<K, V>("BRPOP", s(), key, args...));
      }

      template <class V = auto_t, class Ksrc, class Kdst>
      decltype(auto) brpoplpush(Ksrc const &src, Kdst const &dst,
                                std::int64_t timeout) {
        return run(BulkStringCommand<V>("BRPOPLPUSH", src, dst,
      ...
      template <class K> decltype(auto) llen(K const &key) {
        return run(IntegerCommand("LLEN", key));
      }

      template <class V = auto_t, class K> decltype(auto) lpop(K const &key) {
      ...
      template <class V = auto_t, class K>
      decltype(auto) lrange(K const &key, std::int64_t start,
                            std::int64_t stop) {
        return run(ArrayCommand<V>("LRANGE", key, std::to_string(start),
                                   std::to_string(stop)));
      }
      ...
      template <class K>
      decltype(auto) ltrim(K const &key, std::int64_t start,
                           std::int64_t stop) {
        return run(StatusCommand("LTRIM", key, std::to_string(start),
                                 std::to_string(stop)));
      }

      template <class V = auto_t, class K> decltype(auto) rpop(K const &key) {

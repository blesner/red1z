  ...
  namespace impl {
    ...
    struct BZPopCommand : Command<BZPopCommand<K_, V_>> {
      ...
      static bool process_into(Reply&& r, std::tuple<K, V, double>* out) {
        ...
        if (!OptTupleCommand<K, V, std::string>::process_into(std::move(r), &t)) {
          return false;
        }
    ...
    struct ZRangeWithScoresCommand :
      BasicArrayCommand<std::tuple<auto_type_t<V, std::string>, double>,
                        ...
                        >
    {
      using Base = BasicArrayCommand<std::tuple<auto_type_t<V, std::string>, double>,
                                     ZRangeWithScoresCommand<V>,
                                     std::tuple<std::string, double>
                                     >;

      template <class K>
      ZRangeWithScoresCommand(K const& key, std::int64_t start, std::int64_t stop) :
        Base("ZRANGE", key, std::to_string(start), std::to_string(stop), "WITHSCORES")
      {}
      ...
      static std::int64_t process_into_impl(Reply&& r, OutputIt out) {
        ...
        for (auto i = elements.begin(), end = elements.end(); i != end; i += 2) {
          *out++ = std::make_pair(std::move(i[0]).get<W>(), i[1].floating_point());
        }
        return r.array_size();
      }
    };
    ...
    class SortedSetCommands {
      ...
      decltype(auto) bzpop(char const* cmd, Key const& key, Args const&... args) {
        static constexpr int N = sizeof...(Args);
        static_assert(N > 0, "bzpop*(), missing timeout argument");
        static_assert(std::is_convertible_v<nth_element<N-1, Args...>, std::int64_t>,
                      "timeout must be convertible to integer");
        return run(BZPopCommand<K, V>(converter<nth_arg<N>, int2str>(), cmd, key, args...));
      }
    ...
    public:
      ...
      decltype(auto) zadd(K const& key, Args const&... args) {
        ...
        static_assert(R::first == 0, "usage: zadd(key, [flags], score, member, [score, member ...]");
        CHECK_FLAGS("invalid flags for zadd()", Args..., flags::_nx_xx, flags::_ch, flags::_incr);
        static constexpr bool has_incr_flag = impl::has_flag_v<flags::_incr, Args...>;
        static auto const cvt = converter<every_arg<2>, double2str, R::last+1>();
      ...
      template <class K>
      decltype(auto) zcard(K const& key) { return run(IntegerCommand("ZCARD", key)); }

      template <class K>
      decltype(auto) zcount(K const& key, double min, double max) {
        return run(IntegerCommand("ZCOUNT", key, std::to_string(min), std::to_string(max)));
      }

      template <class K, class M>
      decltype(auto) zincrby(K const& key, double increment, M const& member) {
        return run(FloatCommand("ZINCRBY", key, std::to_string(increment), member));
      }

      template <class Kdst, class K0, class... Args>
      decltype(auto)
      zinterstore(Kdst const& dst, std::int64_t numkeys, K0 const& key, Args const&... args) {
        using R = impl::flags_range<Args...>;
        static_assert(R::empty or R::last == sizeof...(Args), "flags must come last");
        CHECK_FLAGS("invalid flags for zinterstore()", Args..., flags::_weights, flags::_aggregate);
        return run(IntegerCommand("ZINTERSTORE", dst, std::to_string(numkeys), key, args...));
      }

      template <class K>
      decltype(auto) zlexcount(K const& key, double min, double max) {
        return run(IntegerCommand("ZLEXCOUNT", key, std::to_string(min), std::to_string(max)));
      }
      ...
      decltype(auto) zrange(K const& key, std::int64_t start, std::int64_t stop) {
        return run(ArrayCommand<V>("ZRANGE", key, std::to_string(start), std::to_string(stop)));
      }

      template <class V=auto_t, class K>
      decltype(auto)
      zrange(K const& key, std::int64_t start, std::int64_t stop,
